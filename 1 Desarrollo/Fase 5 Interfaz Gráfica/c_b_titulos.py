...
import c_a_aplicacion_inicial, d_b_titulos, z_estilos
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, width, height, nombre, id, encriptacion):
        ...
        self.general = pd.read_csv(BytesIO(self.encriptacion.cargar_csv("Datos_Cifrados/general_csv.enc", context="general_csv")))
        self.peliculas = pd.read_csv(BytesIO(self.encriptacion.cargar_csv("Datos_Cifrados/DatosPeliculasSeries_csv.enc", context="DatosPeliculasSeries_csv")))
        self.pun_com = pd.read_csv(BytesIO(self.encriptacion.cargar_csv(f"Datos_Cifrados/{id}_completo_csv.enc", context=f"{id}_completo_csv")))
        self.pun = pd.read_csv(BytesIO(self.encriptacion.cargar_csv(f"Datos_Cifrados/{id}_csv.enc", context=f"{id}_csv")))
    ...
    def puntuar(self, tid, Titulo_ES, nota):
        ...
        self.encriptacion.datos_descifrados[f"Datos_Cifrados/{self.id}_completo_csv.enc"]= self.pun_com.to_csv(index=True).encode()
        self.encriptacion.guardar_csv(f"Datos_Cifrados/{self.id}_completo_csv.enc", context=f"{self.id}_completo_csv")

        self.pun = pd.concat([self.pun, pd.DataFrame([nueva_fila2])])
        self.encriptacion.datos_descifrados[f"Datos_Cifrados/{self.id}_csv.enc"] = self.pun.to_csv(index=False).encode()
        self.encriptacion.guardar_csv(f"Datos_Cifrados/{self.id}_csv.enc", context=f"{self.id}_csv")
