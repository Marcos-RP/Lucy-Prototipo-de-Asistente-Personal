...
class Pred_Rec:
    ...
    def juntar_usuarios_comunes(self):
        ...
        matriz = self.general
        self.pun_com = pd.read_csv(BytesIO(self.encriptacion.cargar_csv(f"Datos_Cifrados/{self.id}_completo_csv.enc", context=f"{self.id}_completo_csv")))
        self.pun = pd.read_csv(BytesIO(self.encriptacion.cargar_csv(f"Datos_Cifrados/{self.id}_csv.enc", context=f"{self.id}_csv")))
        ...
        for archivo in archivos:
            ...
            if len(partes) == 2 and partes[0].isdigit() and partes[1] == "csv.enc":
                archivo = pd.read_csv(BytesIO(self.encriptacion.cargar_csv(f"Datos_Cifrados/{partes[0]}_csv.enc", context=f"{partes[0]}_csv")))
