...
from . import c_a_aplicacion_inicial, z_estilos
...
class MainWindow(QMainWindow):
    ...
    def comprobarusuario(self, datos):
        ...
        existe = False
        for user in self.encriptacion.datos_descifrados["data/users_json.enc"].values():
    ...
    def registrarusuario(self, datos):
        ...
        archivos = glob.glob(os.path.join("data", "*_completo_csv.enc"))
        numero = str(max((int(os.path.basename(a).split("_")[0]) for a in archivos), default=0) + 1)

        self.encriptacion.datos_descifrados["data/users_json.enc"][numero] = {
            "nombre": nombre,
            "id": numero,
